...
The public packages are [`src/core`](src/core), [`src/test-scene`](src/test-scene), and
[`src/roblox`](src/roblox). Core uses only the documented
...
Compose is Copyright (c) 2026 voidmeld and licensed under the [MIT License](LICENSE). The public
source repository contains no third-party source. The exact Verify dependency is declared in
